...
def generate_requisition_pdf(requisition):
    ...
    fields = [
        ("Unidad Administrativa", requisition.administrative_unit),
        ("Departamento Solicitante", str(requisition.requesting_department)),
        ("Proyecto", str(requisition.project)),
        ("Fuente de Financiamiento", str(requisition.funding_source)),
        ("Unidad Presupuestal", str(requisition.budget_unit)),
        ("Convenios", str(requisition.agreement)),
        ("Categoría", str(requisition.category)),
        ("Fecha de Creación", requisition.created_at.strftime("%d/%b/%Y")),
        ("Motivos de Requisición", requisition.requisition_reason),
        ("Servicio Externo / Académico", str(requisition.external_service)),
        ("Licitación", str(requisition.tender)),
        ("Solicitante", requisition.user.full_name),
    ]
    ...
    # Signatures
    p.setFont("Helvetica-Bold", 12)
    p.drawString(margin_left, y, "Firmas:")
    y -= line_height * 1.5

    sig_labels = [
        f"Solicitó: {requisition.user.full_name}",
        "Revisó: M.I. Arion Ehecatl Juárez Menchaca",
        "Autorizó: M.I. Adrián Isaac Orpinel Uren"
    ]
    p.setFont("Helvetica", 10)
    ...
    # Footer
    footer_text = f"Generado por {requisition.user.full_name} - {requisition.administrative_unit}"
